...
type BlogOCamlPackage = OCamlPackage "blog-types" '[] :>
  ( OCamlModule '["BlogTypes"]
  :> Key
  :> ArticleId
  :> Article
...
instance IsKey TypeParameterRef0 where
  fromKey (Key k) = TypeParameterRef0 k
  toKey (TypeParameterRef0 k) = Key k
...
instance (IsKey a, Arbitrary a, ToADTArbitrary a, Arbitrary b, ToADTArbitrary b) =>
  ToADTArbitrary (Entity a b) where
    toADTArbitrarySingleton Proxy =
      ADTArbitrarySingleton "Blog.Types" "Entity"
        <$> oneof [ ConstructorArbitraryPair "Entity" <$> arbitrary ]

    toADTArbitrary Proxy =
      ADTArbitrary "Blog.Types" "Entity"
